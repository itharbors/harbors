...
body_has_task_heading() {
  local file=$1 line marker fence_character='' fence_length=0
  local opening_fence_regex='^ {0,3}(`{3,}|~{3,})'
  local closing_fence_regex='^ {0,3}(`+|~+)[[:space:]]*$'
  local task_heading_regex='^ {0,3}##[[:space:]]+Task[[:space:]]*$'
...
[[ "$head_sha" =~ ^([0-9a-f]{40}|[0-9a-f]{64})$ ]] || fail 'could not resolve a valid pre-PR HEAD SHA'
...
recorded_pr=$(node -e 'const status = require(process.argv[1]); process.stdout.write(status.pullRequest === null ? "" : String(status.pullRequest.number));' "$repo_root/$status_path")
