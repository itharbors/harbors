...
2. **Task 建档**：先向用户说明计划创建的 `<type>/<slug>` 分支和 `.worktrees/<type>-<slug>` worktree，并取得明确同意后，才从 primary checkout 运行 start；它锁定已 fetch 的 `origin/main`、创建分支与 worktree，并自动 init Task。未获得明确同意时，不得运行 start 或以其他方式创建该分支或 worktree。进入输出的 worktree 后立即填写 `task.md` 中已确认的需求。截止时间、负责人授权和已投入时间都不是跳过 Task 的例外；已有代码但没有 Task 时，先补建并填写 Task，再继续或 finish。不要修改本地 `main` 来让 start 通过，失败须原样报告。
...
5. **收口与 PR**：先完成 `summary.md`，使 implementation、verification、consolidation 等全部内部 stages 终态，并通过 ready gate（`--ready-for-pr`）：`npm run task:status -- check <task-id> --ready-for-pr`。提交所有改动且保持 worktree clean；PR body 文件放在仓库外，包含 `## Summary` 和 `## Testing`，只列实际运行的检查。传给 finish 的 summary 使用不带方括号标签、不带换行且末尾无句号的中文摘要。在运行 finish 以创建或更新 PR 前，向用户汇报准备提交的摘要和实际运行的验证，并取得明确同意；未获得明确同意时，不得运行 finish、push 分支或创建、更新 PR。获得同意后，finish 创建或恢复 PR，添加指向 pre-PR head 上 summary 的不可变 commit 链接，回写 PR 号、自动提交并二次 push。审查要求实质改动时，使用 `rewind` 回退 implementation、verification 或 consolidation，重新实现、验证、更新 summary，再次取得用户同意后再运行 finish；不要沿用过时证据。
