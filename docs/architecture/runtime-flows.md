...
sequenceDiagram
    ...
    E->>N: 绑定 127.0.0.1:17897 并订阅通知状态
...
但 Catalog 和 Tray 仍保留其他 Kit。`npm run dev:web` 可跳过 Electron 单独调试 Web 栈。开发 Gateway 默认监听
18080，Server 监听 13000，Vite 监听 15173；所有页面请求仍从 Gateway 进入。稳定 Electron 继续使用
Gateway 8080、Server 3000、Vite 5173 和 Notification Host 17896；开发 Electron 的 Notification Host 使用 17897。
...
开发 Notification Host 默认监听 `127.0.0.1:17897`，先于 Web 子进程启动，并通过
`HARBORS_NOTIFICATION_PORT` 将实际端口传入子进程。它不属于 Gateway 路由；只启动
`npm run dev:web` 时不会创建该 Host。稳定 Electron 保持监听 `127.0.0.1:17896`。
