...
平台专属通知命令。该 Skill 的唯一分发源是仓库中的
[`.agents/skills/notify-user`](../../.agents/skills/notify-user/)；用户可让 Codex 从
`https://github.com/itharbors/harbors/tree/main/.agents/skills/notify-user` 安装到
`~/.codex/skills/notify-user`，不需要 Harbors 专用安装器。
