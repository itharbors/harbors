...
窗口。保留 `--kit` 单 Kit 模式，并把旧 CE scope 下的所有插件包名统一为
`@itharbors/*`。
...
所有旧 CE scope package/plugin 标识迁移为 `@itharbors/*`，包括内置插件、默认 Kit 插件、
...
1. 仓库受管源码、manifest、测试和文档中不存在旧 CE scope 的插件 package 引用。
...
| 需求 | 自动化证据 |
| --- | --- |
| 插件命名空间统一 | `scripts/lib/plugin-namespace.test.mjs` |
| Kit 发现、单 Kit 筛选与 menuRoot 唯一性 | `scripts/lib/kit-catalog.test.mjs` |
| 稳定 session、bounds 与不可用 Kit 记录 | `scripts/lib/workspace-store.test.mjs` |
| 默认 Electron、`dev:web`、托盘与窗口聚焦 | `scripts/lib/electron-launcher.test.mjs` |
| Editor 配置与菜单状态隔离 | `packages/server/tests/framework/editor.test.ts`、`packages/server/tests/menu/menu.test.ts` |
| APP/Kit 菜单组合与 session 路由 | `packages/server/tests/menu/electron-template.test.ts` |
| Renderer Kit session 与完整菜单 IPC | `packages/client/tests/core/transport.test.ts`、`packages/client/tests/menu/runtime.test.ts` |
