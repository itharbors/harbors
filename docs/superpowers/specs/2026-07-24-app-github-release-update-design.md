...
GitHub Provider 在选择 prerelease 时会把完整 Release Tag 交给 SemVer 解析，因此 Tag 必须保持
`v<canonical-semver>`；带目录前缀的 `app/v...` 会使 Preview Release 无法被发现。Kit Tag 已使用
`kit/<name>/...` 命名空间，所以两类触发器仍然互不重叠。
...
- 已发布的 `v*` Tag 和 Release 不允许覆盖、删除或替换资产。
...
更新器只在 `app.isPackaged` 且当前 macOS 可执行文件通过 **Developer ID Application** 签名验证时
启用。开发构建、测试构建、ad-hoc 签名和未签名目录构建不访问更新源；环境变量仅作为额外的验收
opt-out，不能充当正式发布身份。
...
- `publish-app.yml`：只响应 updater 可直接解析的 `v*`，负责最小调用入口。
...
并限制最小权限。`app-publish-v1` 与 `v*` 都加入 Tag ruleset，禁止更新和删除；禁止存在
`app-publish-v1` 同名分支。
...
- app workflow 只匹配 `v*`，Kit workflow 只匹配 `kit/*/v*`。
...
10. 确认 `v*` 不触发 Kit workflow，`kit/*/v*` 不触发 app workflow。
