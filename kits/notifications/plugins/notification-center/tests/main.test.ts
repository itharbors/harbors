...
describe('notification-center plugin main', () => {
  ...
  it('maps UI methods to the loopback Notification Host', async () => {
    const definition = await loadDefinition();
    const requests: Array<{ url: string; init?: RequestInit }> = [];
    vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
      requests.push({ url, init });
      if (init?.method === 'DELETE') return new Response(null, { status: 204 });
      if (url.endsWith('/read-all')) return jsonResponse({ unreadCount: 0 });
      if (url.endsWith('/read')) return jsonResponse({ id: 'a/b', read: true });
