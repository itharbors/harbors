...
- **Kit 独立发布**：产品分支通过固定 reusable workflow 自动生成 Release、SBOM、Artifact Attestation 与 GitHub Pages Registry 投影。
...
安全重启激活。官方 Kit 产品分支完成迁移并首次发布前，Registry 可能为空。
...
npm run kit:publish -- aggregate \
  --entries-directory ./registry/entries \
...
Kit 分支的 caller 模板、固定 `kit-publish-v1` reusable workflow 和 `kit-registry` Pages 模板已经
包含在仓库中。Preview push 自动发布 prerelease 并更新 preview 投影；Stable Tag 经过
`kit-stable` Environment 审批后创建不可变 Release 和 Registry 审核 PR。具体初始化要求见
[Kit 制品与 Registry 指南](./docs/guides/kit-artifacts.md)。

Framework 变更使用 `change-workflow` 并以 `main` 为 base；Kit 变更使用仓库本地 `kit-workflow`，
固定从 `kit/<kit>` 创建 `kit-change/<kit>/<type>/<slug>` worktree，PR 也只能回到对应产品分支。
SQLite、MySQL、Notifications 与 Registry 的首次独立快照可分别由
`scripts/migrate-kit-product.mjs` 和 `scripts/migrate-kit-registry.mjs` 生成；迁移器不会推送分支或
触发发布。
