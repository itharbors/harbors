...
- **多 Kit 桌面宿主**：Electron 默认发现全部 Kit，只打开默认 Kit；其他 Kit 在用户从系统托盘首次打开时才创建窗口和 session。
...
Electron 会启动以下开发服务，加载各 Kit 声明的应用级启动插件，只创建默认 Kit 的可见窗口，
并在系统托盘中列出其他可按需打开的 Kit：
...
| Notification Host | http://127.0.0.1:17896 | 接收本机通知、维护未读状态并驱动桌面提示 |

Gateway 会把 `/api/*` 和 `/sse/*` 转发给 Server，并把其他请求转发到 Client 开发服务。需要直接在浏览器中调试时运行 `npm run dev:web`，再访问 [http://localhost:8080](http://localhost:8080)。
...
`npm run dev` 与 `npm run electron` 等价。默认进入多 Kit 模式；使用 `--kit` 时只启动指定 Kit，菜单也恢复为单 Kit 平铺形式：
