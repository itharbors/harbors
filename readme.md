...
自动打开默认 Kit。单击或右键托盘
图标，从列表选择 Default、Agent Guard、CSV、SQLite、MySQL 或 Notifications；首次选择会按需加载，之后再次选择只会打开或
...
Agent Guard、CSV、SQLite、MySQL、Notifications 分别维护在 `kits/agent-guard`、`kits/csv`、`kits/sqlite`、`kits/mysql`、`kits/notifications`。
