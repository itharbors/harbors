...
describe('SQLite Explorer object snapshot owner', () => {
  afterEach(() => {
    vi.resetModules();
    ...
    const request = vi.fn(async (_plugin: string, method: string) => {
      if (method === 'getSchema') {
        return {
          ...
          schemaRevision: 4,
          dataRevision: 7,
          objects: [{ name: 'orders', kind: 'view' }, { name: 'users', kind: 'table' }],
        };
      }
  ...
  it('preserves valid selection, repairs invalid selection, and ignores a stale refresh', async () => {
    const definition = await loadDefinition();
    const broadcast = vi.fn();
    ...
    let schema = {
      ...
      schemaRevision: 1,
      dataRevision: 1,
      objects: [{ name: 'users', kind: 'table' }, { name: 'audit', kind: 'view' }],
    };
  ...
  it('waits for an in-flight connection refresh before validating a selection', async () => {
    ...
    resolveSchema?.({
      ...
      schemaRevision: 5,
      dataRevision: 5,
      objects: [{ name: 'smoke_items', kind: 'table' }],
    });
  ...
  it('does not carry a same-named selection across connection revisions', async () => {
    ...
    let schema = {
      ...
      schemaRevision: 1,
      dataRevision: 1,
      objects: [{ name: 'shared', kind: 'table' }],
    };
    const request = vi.fn(async () => schema);
    definition.lifecycle?.load?.({ message: { request, broadcast: vi.fn() } });
    await definition.methods.onConnectionChanged({ connected: true, connectionRevision: 7, schemaRevision: 1 });

    schema = {
      connectionRevision: 8,
      schemaRevision: 2,
      dataRevision: 2,
      objects: [{ name: 'first_table', kind: 'table' }, { name: 'shared', kind: 'view' }],
    };
    await definition.methods.onConnectionChanged({ connected: true, connectionRevision: 8, schemaRevision: 2 });
