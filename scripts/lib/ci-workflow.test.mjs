...
test('Kit CI builds every Server workspace dependency before each Kit lifecycle and never publishes', async () => {
  ...
  const lifecycleBuildIndex = checkKit.indexOf('npm run build -w @itharbors/plugin -w @itharbors/plugin-types -w @itharbors/kit-core -w @itharbors/kit-cli -w @itharbors/host-security -w @itharbors/server');
...
test('root test delegates Kit work to descriptor-driven lifecycle scripts', async () => {
  ...
  assert.equal(
    ...
    'npm run test:toolchain && npm run test -w @itharbors/plugin && npm run test:framework:prepared',
  );
  assert.match(
