...
const descriptors = Object.freeze([
  Object.freeze({ slug: 'zeta', directory: '/repo/kits/zeta', distribution: 'market' }),
  Object.freeze({ slug: 'alpha', directory: '/repo/kits/alpha', distribution: 'builtin' }),
]);
...
test('creates lifecycle commands from descriptors in canonical slug order', () => {
  assert.deepEqual(createKitMatrixPlan({ action: 'build', descriptors }), [
    { slug: 'alpha', command: 'build', directory: '/repo/kits/alpha' },
    { slug: 'zeta', command: 'build', directory: '/repo/kits/zeta' },
  ]);
  assert.deepEqual(createKitMatrixPlan({ action: 'test', slugs: ['zeta'], descriptors }), [
    { slug: 'zeta', command: 'test', directory: '/repo/kits/zeta' },
  ]);
  assert.throws(
...
test('prepares the fixed Kit runner and executes the exact full check lifecycle for builtin and market Kits', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ['build', '/repo/kits/alpha'],
      ['test', '/repo/kits/alpha'],
      ['validate', '/repo/kits/alpha'],
      ['pack', '/repo/kits/alpha', '--output', '/tmp/check-alpha/alpha.hkit'],
      ['inspect', '/tmp/check-alpha/alpha.hkit', '--json'],
      ['build', '/repo/kits/zeta'],
      ['test', '/repo/kits/zeta'],
      ['validate', '/repo/kits/zeta'],
      ['pack', '/repo/kits/zeta', '--output', '/tmp/check-zeta/zeta.hkit'],
  ...
  assert.deepEqual(removed, ['/tmp/check-alpha', '/tmp/check-zeta']);
});
...
test('cleans failed checks, continues later Kits, and reports deterministic aggregate results', async () => {
  ...
  await assert.rejects(
    runKitMatrix({
      ...
      run: async (file, args) => {
        ...
        if (args.includes('test') && args.includes('/repo/kits/alpha')) {
  ...
  assert.deepEqual(removed, ['/tmp/failure-0', '/tmp/failure-1']);
  assert.ok(calls.some((call) => call.includes('/repo/kits/zeta')));
  assert.equal(calls.some((call) => call.includes('validate') && call.includes('/repo/kits/alpha')), false);
});
