...
describe('PluginModule', () => {
  ...
  it('keeps registration state instance-scoped', async () => {
    ...
    const pluginDir = mkPlugin(root, 'menu', 'menu');
  ...
  it('loads builtin menu plugin from main/dist entry', async () => {
    ...
    expect(editor.plugin.getInfo('menu')?.path).toContain(`${path.sep}plugins${path.sep}menu`);
    expect(editor.plugin.listLoaded()).toContain('menu');
    expect(editor.plugin.callPlugin('menu', 'newSession')).toEqual({ ok: true, action: 'newSession' });
  });
